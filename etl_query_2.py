...
from pyspark.sql.functions import col, to_timestamp, desc, asc
import time
...
# ✅ Find youngest and oldest 5 active employees (using 'is_active' column)
print("🧑‍💼 Finding youngest and oldest 5 active employees...")
active_employees = df_transformed.filter(col("is_active") == True)

youngest_5 = active_employees.orderBy(asc("age")).limit(5)
oldest_5 = active_employees.orderBy(desc("age")).limit(5)

print("👶 Youngest 5 employees:")
youngest_5.show(truncate=False)

print("👴 Oldest 5 employees:")
oldest_5.show(truncate=False)
